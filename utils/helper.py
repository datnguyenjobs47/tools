...
def load_scraper_basic_config(platform: str) -> Dict[str, Any]:
    """Load platform-specific scraper configuration from YAML"""
    
    current_dir = Path(__file__).parent
    config_path = current_dir / ".." / "configs" / "scraper_basic_config.yml"
    ...
    with open(config_path, 'r', encoding='utf-8') as f:
        ...
        for platform_config in data.get("mall"):
...
def get_price_ranges(config: Dict[str, Any]) -> List[Tuple[int, int]]:
    """Extract price ranges from config"""
    return [tuple(range_pair) for range_pair in config['price_ranges']]


def get_mobile_user_agent_list():
    return [
...
def get_desktop_user_agent_list():
    return [
